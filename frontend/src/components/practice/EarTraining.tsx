...
export default function EarTraining() {
  ...
  return (
    <div className="card p-6">
      <div>
        <p className="text-sm font-semibold text-brand-gold">Ear Training</p>
      ...
      <div className="mt-6 grid gap-2 sm:grid-cols-3 lg:grid-cols-4">
        {notes.map((note) => (
          <button
            ...
            className="rounded-2xl border border-stroke bg-black/50 px-3 py-2 text-xs font-semibold text-ink transition hover:border-brand-gold"
          >
            {note}
          </button>
        ))}
